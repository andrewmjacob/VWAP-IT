...
@app.command()
def run_reddit_connector(
    mode: str = typer.Option("shadow", help="shadow or emit"),
    interval: int = typer.Option(120, help="Seconds between fetches"),
    subreddits: str = typer.Option("wallstreetbets", help="Comma-separated subreddits"),
):
    """Run the Reddit connector in a continuous loop.
    
    Fetches posts from r/wallstreetbets and other finance subreddits,
    extracts ticker mentions, and creates SOCIAL.MENTIONS events.
    """
    ...
    typer.echo(f"Starting Reddit connector (mode={mode}, subreddits={subreddit_list}, interval={interval}s)")
...
@app.command()
def consume_events(
    batch_size: int = typer.Option(10, help="Messages per batch"),
    max_iterations: int = typer.Option(0, help="Max iterations (0=infinite)"),
):
    """Consume and process events from SQS.
    
    Reads events from the SQS queue and processes them:
    - SOCIAL.MENTIONS: Extracts ticker sentiment artifacts
    - Other events: Acknowledged and logged
    """
    from tip.consumers.event_processor import EventProcessor
    from tip.consumers.base import ConsumerConfig
    ...
    consumer = EventProcessor(config, s.PG_DSN)
    typer.echo(f"Starting event consumer (batch_size={batch_size})")
    consumer.run(max_iterations=max_iterations)
